...
namespace HolographicApp1
{
    ...
    {
        private HolographicApp1Main main;
        ...
        public void SetWindow(CoreWindow window)
        {
            ...
            // 現在のビューのコアウィンドウ用のホログラフィックスペースを作成します。
            // このホログラフィック空間によって作成されたホログラフィックフレームを表示すると、アプリが排他モードになります。
            holographicSpace = HolographicSpace.CreateForCoreWindow(window);

            // DeviceResourcesクラスは、Direct3Dデバイスを作成するために、ホログラフィック空間（使用可能な場合）からの優先DXGIアダプタIDを使用します。 
            // HolographicSpaceは、このID3D11Deviceを使用して、スワップチェーンなどのデバイスベースのリソースを作成および管理します。...
